...
namespace Unity.Cinemachine3.Authoring
{
    ...
    /// Instantiate as needed, don't keep.
    /// </summary>
    public struct ConvertEntityHelper
    {
        /// <summary>Constructor to wrap a transform</summary>
        /// <param name="t">The transform that should also be an Entity</param>
        /// <param name="createEntity">Force creation of entity if true</param>
        /// GML todo something better here
        public ConvertEntityHelper(Transform t, bool createEntity = true)
        {
            Transform = t;
            EntityManager = World.Active?.EntityManager;
            Entity = Entity.Null;
            if (t != null && EntityManager != null)
            {
                var convert = t.GetComponent<CM_ConvertToEntity>();
                if (convert == null && createEntity)
                    convert = t.gameObject.AddComponent<CM_ConvertToEntity>();
                if (convert != null)
                    Entity = convert.Entity;
            }
        }

        public static void DestroyEntity(Transform t)
        {
            var convert = t?.GetComponent<CM_ConvertToEntity>();
            if (convert != null)
                convert.DestroyEntity();
        }
        ...
        {
            return !IsNull && EntityManager.HasComponent<T>(Entity);
        }
