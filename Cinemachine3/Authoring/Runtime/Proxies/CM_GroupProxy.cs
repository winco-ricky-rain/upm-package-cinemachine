...
namespace Unity.Cinemachine3.Authoring
{
    ...
    [SaveDuringPlay]
    [ExecuteAlways]
    public class CM_GroupProxy : CM_DynamicBufferBase<CM_GroupBufferElement>
    {
        [Serializable] public struct Target
        {
            /// <summary>The target objects.  This object's position and orientation will contribute to the
            ...
            Entity entity, EntityManager dstManager, GameObjectConversionSystem conversionSystem)
        {
            ...
            {
                var t = m_Targets[i];
                //var goh = new GameObjectEntityHelper(t.target, false);
                scratchList.Add(new CM_GroupBufferElement { /*target = goh.Entity,*/ weight = t.weight });
            }
            SetValue(scratchList);
        ...
        protected override bool ElementsEqual(CM_GroupBufferElement a, CM_GroupBufferElement b)
        {
        ...
        public void AddMember(Transform t, float weight)
        {
            if (m_Targets == null)
                m_Targets = new List<Target>();
            m_Targets.Add(new Target { target = t, weight = math.max(0, weight) } );
            OnValidate();
        }
        ...
        void DoUpdate()
        {
            CM_VcamBase.EnsureTargetCompliance(transform);

            // Ensure target compliance
            var value = Value;
            var count = m_Targets.Count;
            for (int i = 0; i < count; ++i)
            {
                var t = m_Targets[i];
                var e = CM_VcamBase.EnsureTargetCompliance(t.target);
                scratchList.Add(new CM_GroupBufferElement { target = e, weight = t.weight });
            }
            if (!IsSame(scratchList))
            {
                SetValue(scratchList);
                OnValidate();
            }
